...
class Packer {
	constructor(impackFile, options) {
		...
		this.stagedir = path.resolve(this.output, this.config.get('name'));
		this.hooksdir = path.resolve(this.output, 'hooks');
		this.packroot = options.packroot || '/opt';
		this.packhome = path.resolve(this.packroot, this.config.get('name'));
		this.update();
	}
	...
	_parsePackOptions(options = {}) {
		...
		options = Object.assign({
			// chdir: this.stagedir,
			architecture,
			package: path.resolve(this.output, `${opts.name}_${opts.version}_${architecture}.deb`),
		}, opts, options);
	...
	/**
	 *
	 * @param {Array|String|Object} resources files and directories you want to include in the package. Example: "from=target"
	 * @param {Object} [options] The options for fpm. Details see: fpm --help
	 * @returns {Promise.<*>}
	 */
	async pack(resources, options = {}) {
		...
		resources.push(`${this.stagedir}=${this.packroot}`);
		options = this._parsePackOptions(options);

		const args = [...utils.argumentize(options), ...resources];
